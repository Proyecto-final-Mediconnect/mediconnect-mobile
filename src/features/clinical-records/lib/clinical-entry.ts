...
export function readEntryFields(entry: ClinicalEntry): CampoLegible[] {
  ...
  return Object.entries(content as Record<string, unknown>)
    .filter(
      ([clave, valor]) => !NO_CLINICO.has(clave) && typeof valor === 'string' && valor.length > 0,
    )
...
/**
 * `2026-08-27T12:34:00.000Z` → `27 de agosto de 2026 · 09:34`, en hora de
 * Argentina. Mismo formato que la web; la hora va completa porque en un asiento
 * clínico el momento es parte del registro. A mano y no con `Intl`, por lo
 * mismo que en los turnos.
 */
export function formatEntryDate(iso: string): string {
  const { date, time } = enArgentina(new Date(iso));
  const [y, m, d] = date.split('-').map(Number);
  return `${d} de ${MESES[(m ?? 1) - 1]} de ${y} · ${time}`;
}

/** `2026-08-27T12:34:00.000Z` → `27 ago 2026`, para las tarjetas de la lista. */
export function formatEntryShortDate(iso: string): string {
  const { date } = enArgentina(new Date(iso));
  const [y, m, d] = date.split('-').map(Number);
  return `${d} ${(MESES[(m ?? 1) - 1] ?? '').slice(0, 3)} ${y}`;
}

export function authorName(entry: ClinicalEntry): string {
  const p = entry.professional;
  // Perder el nombre es mejor que perder el registro.
  return p ? `${p.firstName} ${p.lastName}` : 'un profesional sin perfil cargado';
}
